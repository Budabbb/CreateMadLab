...
void main() {
    ...
    if (gl_Color.a != 0.8) {
        float y = position.y;
        float z = position.z;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.z) / 256.0)) / 24.0 * fadeIntensity;
    ...
    if (gl_Color.a != 0.8) {
        float y = position.y;
        float x = position.x;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.x) / 384.0)) / 24.0 * fadeIntensity;
