...
void main() {
    ...
    if (gl_Color.a == 0.4) {
        float y = position.y;
        float z = position.z;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.z) / 256.0)) / 48.0 * fadeIntensity; 
    ...
    if (gl_Color.a == 0.4) {
        float y = position.y;
        float x = position.x;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.x) / 384.0)) / 48.0 * fadeIntensity; 
    ...
    if (gl_Color.a != 0.4) {
        float y = position.y;
        float z = position.z;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.z) / 9999.0)) / 48.0 * fadeIntensity; 
        position.y = z*sin(om)+y*cos(om);
        position.z = z*cos(om)-y*sin(om);
    }
    if (gl_Color.a != 0.4) {
        float y = position.y;
        float x = position.x;
        float om = cos(2.0 * PI * (frameTimeCounter / 5.50 + (worldpos.x) / 9999.0)) / 48.0 * fadeIntensity; 
        position.y = x*sin(om)+y*cos(om);
        position.x = x*cos(om)-y*sin(om);
    }
